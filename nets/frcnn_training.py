...
class FasterRCNNTrainer(nn.Module):
    ...
    def forward_fisher(self, n, img_size, imgs, bboxes, labels, scale):
        # n = imgs.shape[0]
        # img_size = imgs.shape[2:]
        # -------------------------------#
        #   获取公用特征层
        # -------------------------------#
        # imgs = torch.autograd.Variable(imgs, volatile=False)
        # labels = torch.autograd.Variable(torch.Tensor(labels), volatile=False)
        base_feature = self.model_train(imgs, mode='extractor')
    ...
    def forward_ewc(self, fisher, rpn_loc_weight, imgs, bboxes, labels, scale):
        n = imgs.shape[0]
        img_size = imgs.shape[2:]
        # -------------------------------#
        #   获取公用特征层
        ...
        loss_reg = 0
        for (name_incre, param_incre), (name_old, para_old) in zip(self.model_train.module.rpn.loc.named_parameters(),
                                                                   rpn_loc_weight.items()):
            assert name_old == name_incre
            loss_reg += torch.sum(fisher[name_old].cuda() * (para_old.cuda() - param_incre).pow(2)) / 2
    ...
    def train_step_fisher(self, n, img_size, imgs, bboxes, labels, scale, fp16=False, scaler=None):
        self.optimizer.zero_grad()
        if not fp16:
            losses = self.forward_fisher(n, img_size, imgs, bboxes, labels, scale)
            losses[-1].backward()
            self.optimizer.step()
        else:
            from torch.cuda.amp import autocast
    ...
    def train_step_ewc(self, fisher, rpn_loc_weight, imgs, bboxes, labels, scale, fp16=False, scaler=None):
        self.optimizer.zero_grad()
        if not fp16:
            losses = self.forward(imgs, bboxes, labels, scale)
            losses[-1].backward()
            ...
            from torch.cuda.amp import autocast
            with autocast():
                losses, loss_reg = self.forward_ewc(fisher, rpn_loc_weight, imgs, bboxes, labels, scale)
                all_loss = losses[-1] + loss_reg
